module FIFO(FIFO_if.DUT fifo_if);
	...
	always @(posedge fifo_if.clk or negedge fifo_if.rst_n) begin
		if (!fifo_if.rst_n) begin
			wr_ptr <= 0;
			fifo_if.wr_ack  <= 0;  //Fix 3
			fifo_if.overflow <= 0;  //Fix 3
	...
	always @(posedge fifo_if.clk or negedge fifo_if.rst_n) begin
		if (!fifo_if.rst_n) begin
			rd_ptr <= 0;
			fifo_if.underflow <= 0; //Fix 4
        	fifo_if.data_out  <= 0; //Fix 4
		end
    ...
    // assign fifo_if.underflow = (fifo_if.empty && fifo_if.rd_en)? 1 : 0; // Fix 1
	assign fifo_if.almostfull = (count == FIFO_DEPTH-1)? 1 : 0; // Fix 2
